...
DATASET_PATH = Path(sys.argv[1] if len(sys.argv) > 1 else './datasets/own_data/')
RESULT_PATH = Path('./intermediate_datafiles/')
# RESULT_FNAME = sys.argv[2] if len(sys.argv) > 2 else 'chapter2_result.csv'
RESULT_FNAME = sys.argv[2] if len(sys.argv) > 2 else 'own_data_result.csv'


# Set a granularity (the discrete step size of our time series data). We'll use a course-grained granularity of one
# instance per minute, and a fine-grained one with four instances per second.
GRANULARITIES = [500, 1000, 30000]
...
for milliseconds_per_instance in GRANULARITIES:
    ...
    # dataset.add_numerical_dataset('pressure_phone.csv', 'Time (s)', ['pressure'], 'avg', 'press_phone_')
    ...
    # Plot all data
    # DataViz.plot_dataset(dataset, ['acc_', 'gyr_', 'mag_', 'press_phone_', 'label'],
